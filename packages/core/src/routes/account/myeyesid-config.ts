...
import { AccountCenterControlValue, userMfaDataGuard, userMfaDataKey } from '@myeyesid/schemas';
import { z } from 'zod';
...
export default function myeyesidConfigRoutes<T extends UserRouter>(...args: RouterInitArgs<T>) {
  ...
  router.get(
    `${accountApiPrefix}/myeyesid-configs`,
    koaGuard({
      response: z.object({
        mfa: z.object({
          skipped: z.boolean(),
    ...
    async (ctx, next) => {
      ...
      // Currently, only the MFA skip state is exposed in myeyesid_config
