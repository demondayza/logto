const enterprise_sso = {
  ...
  placeholder_description:
    'MyEyesID 提供了许多内置的企业身份提供者，与之连接，与此同时你可以使用 SAML 和 OIDC 协议创建自己的企业身份提供者。',
  create_modal: {
    title: '添加企业连接器',
  ...
  basic_info: {
    ...
    oidc: {
      ...
      redirect_uri_field_custom_domain_description:
        '如果你在 MyEyesID 中使用多个<a>自定义域名</a>，务必将所有对应的回调 URI 都添加到 IdP 中，以确保 SSO 在每个域名上都能正常工作。\n\n默认的 MyEyesID 域名 (*.myeyesid.app) 始终有效，只有在你也希望支持该域名下的 SSO 时才需要包含它。',
    },
  },
  attribute_mapping: {
    title: '属性映射',
    description: '需要 `id` 和 `email` 来同步用户配置文件。在 IdP 中输入以下声明名称和值。',
    col_sp_claims: '服务提供商（MyEyesID）的值',
