const application_details = {
  ...
  settings_description:
    '“应用”是注册的软件或服务，可以访问用户信息或代表用户执行操作。应用可以帮助 MyEyesID 识别是谁在请求什么，并负责处理登录和授权。请填写认证所需的必填字段。',
  integration: '集成',
  integration_description:
    '使用由 Cloudflare 边缘网络提供动力的 MyEyesID 安全工作者部署，实现世界各地的一流性能和 0ms 冷启动。',
  ...
  myeyesid_endpoint: 'MyEyesID 端点',
  application_id: '应用 ID',
  application_id_tip:
    '应用的唯一标识，通常由 MyEyesID 生成。等价于 OpenID Connect 中的 <a>client_id</a>。',
  ...
  always_issue_refresh_token_label:
    '启用此配置将允许 MyEyesID 始终颁发刷新令牌，无论身份验证请求中是否呈现 `prompt=consent`。 然而，除非必要，否则不推荐这样做，因为它与 OpenID Connect 不兼容，可能会导致问题。',
  refresh_token_ttl: '刷新令牌有效期（天）',
  refresh_token_ttl_tip:
    '可用于请求新访问令牌的刷新令牌在过期之前的时间段。访问令牌请求将把刷新令牌的时效延长到此值。',
  rotate_refresh_token: '轮换刷新令牌',
  rotate_refresh_token_label:
    '启用后，当原先的刷新令牌的时效已经过去 70%，或者满足一定条件时，MyEyesID 将会为访问令牌请求发放新的刷新令牌。<a>了解更多</a>',
  rotate_refresh_token_label_for_public_clients:
    '启用后，MyEyesID 将为每个令牌请求颁发一个新的刷新令牌。<a>了解更多</a>',
  ...
  app_domain_description_1: '随时使用由 MyEyesID 提供支持的 {{domain}} 域名，永久有效。',
  ...
  third_party_settings_description:
    '将第三方应用程序与 MyEyesID 集成为你的身份提供者 (IdP)，使用 OIDC / OAuth 2.0，并提供用户授权同意界面。',
  ...
  permissions: {
    ...
    permission_description_tips:
      '当 MyEyesID 作为第三方应用程序中的身份提供者（IdP）用于授权时，用户被要求授权时，此描述会显示在授权屏幕上。',
    user_title: '用户',
  ...
  saml_idp_name_id_format: {
    ...
    persistent: '持久性',
    persistent_description: '使用 MyEyesID 用户 ID 作为名称 ID',
    transient: '临时性',
    transient_description: '使用一次性用户 ID 作为名称 ID',
    unspecified: '未指定',
    unspecified_description: '使用 MyEyesID 用户 ID 作为名称 ID',
  ...
  saml_app_attribute_mapping: {
    name: '属性映射',
    title: '基础属性映射',
    description: '添加属性映射以从 MyEyesID 同步用户资料到你的应用程序。',
