const enterprise_sso_details = {
  ...
  service_provider_property_description:
    '在你的身份提供者中使用{{protocol}}设置应用程序集成。输入 MyEyesID 提供的详细信息。',
  attribute_mapping_title: '属性映射',
  attribute_mapping_description:
    '通过在身份提供者或 MyEyesID 端配置用户属性映射，从身份提供者同步用户配置文件。',
  saml_preview: {
    sign_on_url: '登录URL',
    entity_id: '发行者',
  ...
  idp_initiated_auth_config: {
    ...
    redirect_uri_tooltip:
      '用户成功登录后将被重定向的 URI。MyEyesID 将在授权请求中使用此 URI 作为 OIDC 重定向 URI。为 IdP 发起的 SSO 认证流程使用专用 URI 以获得更好的安全性。',
  ...
  trust_unverified_email_tip:
    'Entra ID（OIDC）连接器不返回 `email_verified` 声明，这意味着来自 Azure 的电子邮件地址不一定经过验证。默认情况下，MyEyesID 将不会将未验证的电子邮件地址同步到用户配置文件。只有在你信任来自 Entra ID 目录的所有电子邮件地址时才启用此选项。',
  trust_unverified_email_tip_oidc:
    'OIDC 连接器可能不会返回 `email_verified` 声明，这意味着来自身份提供者的电子邮件地址不一定经过验证。默认情况下，MyEyesID 不会将未验证的电子邮件地址同步到用户配置文件。只有在你信任来自身份提供者的所有电子邮件地址时才启用此选项。',
  offline_access: {
    label: '刷新访问令牌',
    description:
