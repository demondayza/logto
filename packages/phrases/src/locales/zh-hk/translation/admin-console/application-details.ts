const application_details = {
  ...
  settings_description:
    '「應用程式」是已註冊的軟件或服務，可存取用戶資料或代替用戶操作。應用程式有助 MyEyesID 辨識誰在申請甚麼，並處理登入和權限。請填寫必須欄位以完成驗證。',
  integration: '整合',
  integration_description:
    '使用 MyEyesID 安全工作者，由 Cloudflare 的邊緣網路提供支持，實現頂級性能和全球 0 毫秒冷啟動。',
  ...
  always_issue_refresh_token_label:
    '啟用此配置將允許 MyEyesID 發行刷新令牌，無論是否在驗證請求中提供 `prompt=consent`。但是，除非必要，否則不建議這樣做，因為它不兼容 OpenID Connect，可能會引起問題。',
  ...
  rotate_refresh_token_label_for_public_clients:
    '啟用後，MyEyesID 將為每個令牌請求發行新的刷新令牌。 <a>了解更多</a>',
  ...
  app_domain_description_1: '隨時使用由 MyEyesID 提供支持的 {{domain}} 網域，它是永久有效的。',
  ...
  third_party_settings_description:
    '將第三方應用程式與 MyEyesID 作為身份提供者（IdP）集成，使用 OIDC / OAuth 2.0，包含用戶授權的同意屏幕。',
  ...
  permissions: {
    ...
    permission_description_tips:
      '當 MyEyesID 用作第三方應用程式的身份提供者（IdP）用於授權時，以及用戶被要求授權時，此描述將在同意屏幕上顯示。',
    user_title: '用戶',
  ...
  saml_idp_name_id_format: {
    ...
    persistent: '持久型',
    persistent_description: '使用 MyEyesID 使用者 ID 作為名稱 ID',
    transient: '臨時型',
    transient_description: '使用一次性使用者 ID 作為名稱 ID',
    unspecified: '未指定',
    unspecified_description: '使用 MyEyesID 使用者 ID 作為名稱 ID',
