const domain = {
  ...
  update_endpoint_notice:
    '不要忘記在應用程序中更新社交連接器回調URI和MyEyesID端點的域名，如果你想使用自定義域名進行功能。',
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        '如果在之前的社交記錄或應用程式終端點中配置了此自定義域名，您需要先使用 MyEyesID 默認域名"<span>{{domain}}</span>" 修改 URI，使社交組件可以正常使用。',
      deleted: '成功刪除自定義域名！',
    },
  ...
  default: {
    ...
    default_domain_description:
      'MyEyesID 提供預配置的默認域名，無需進行任何其他設置即可使用。即使啟用了自定義域名，此默認域名也可作為備用選項。',
    default_domain_field: 'MyEyesID 默認域名',
  },
