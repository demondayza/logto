const enterprise_sso = {
  ...
  placeholder_description:
    'MyEyesID 提供了許多內置的企業身份提供者來連接，同時您可以使用 SAML 和 OIDC 協定創建自己的。',
  create_modal: {
    title: '新增企業連接器',
  ...
  basic_info: {
    ...
    oidc: {
      ...
      redirect_uri_field_custom_domain_description:
        '如果你在 MyEyesID 使用多個<a>自訂網域</a>，請務必把所有對應的回調 URI 加入 IdP，確保 SSO 在每個網域都能運作。\n\n預設的 MyEyesID 網域 (*.myeyesid.app) 一直有效；只有在你也想支援該網域下的 SSO 時才需要包含它。',
    },
  },
  attribute_mapping: {
    title: '屬性映射',
    description: '同步用戶概要從 IdP 需要 `id` 和 `email`。在你的 IdP 中輸入以下聲明名稱和值。',
    col_sp_claims: '服務提供者（MyEyesID）的值',
