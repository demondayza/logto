const sign_up_and_sign_in = {
  ...
  social_sign_in: {
    ...
    automatic_account_linking_tip:
      '啟用後，如果用戶使用新的社交身份登錄，並且恰好有一個具有相同標識（例如，郵件地址）的現有帳戶，MyEyesID 將自動將該社交身份鏈接到該帳戶。用戶將不會被要求選擇是否鏈接帳戶。',
  ...
  advanced_options: {
    ...
    unknown_session_redirect_url_tip:
      '有時，MyEyesID 可能無法識別用戶在登錄頁面的 Session，比如 Session 過期或用戶書籤或分享錄入連結時。預設顯示"未知 Session" 404 錯誤。為增強用戶體驗，設定一個回退 URL，將用戶重定向回應用程式並重啟身份認證。',
  },
};
