const domain = {
  ...
  update_endpoint_notice:
    'Não se esqueça de atualizar o domínio para o URI de retorno do conector social e o endpoint de MyEyesID na sua aplicação se quiser usar um domínio personalizado para as funcionalidades.',
  error_hint:
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        'Se já configurou este domínio personalizado no seu fornecedor de conector social ou extremidade de aplicação antes, terá de modificar a URI para o domínio padrão do MyEyesID "<span>{{domain}}</span>" primeiro. Isto é necessário para que o botão de início de sessão social funcione corretamente.',
  ...
  default: {
    ...
    default_domain_description:
      'MyEyesID oferece um domínio predefinido pré-configurado, pronto para usar sem qualquer configuração adicional. Este domínio predefinido serve como opção de backup mesmo que tenha ativado um domínio personalizado.',
    default_domain_field: 'Domínio predefinido da MyEyesID',
  },
