const application_details = {
  ...
  settings_description:
    'Uma aplicação é um software ou serviço registado que pode aceder às informações do utilizador ou agir em seu nome. As aplicações ajudam o MyEyesID a identificar quem está a pedir o quê e tratam do início de sessão e das permissões. Preencha os campos obrigatórios para a autenticação.',
  integration: 'Integração',
  integration_description:
    'Implemente com os trabalhadores seguros da MyEyesID, alimentados pela rede de borda da Cloudflare para um desempenho de primeira classe e inícios instantâneos a 0ms em todo o mundo.',
  ...
  application_id_tip:
    'O identificador exclusivo da aplicação normalmente gerado pelo MyEyesID. Também representa “<a>client_id</a>” no OpenID Connect.',
  ...
  always_issue_refresh_token_label:
    'Ao ativar essa configuração, a MyEyesID sempre emitirá tokens de atualização, independentemente de `prompt = consent` ser apresentado na solicitação de autenticação. No entanto, essa prática é desencorajada, a menos que seja necessária, pois não é compatível com OpenID Connect e pode causar problemas.',
  ...
  rotate_refresh_token_label:
    'Quando ativado, o MyEyesID emitirá um novo token de atualização para solicitações de token quando 70% do tempo de vida original (TTL) tiver passado ou certas condições forem atendidas. <a>Saiba mais</a>',
  rotate_refresh_token_label_for_public_clients:
    'Quando ativado, o MyEyesID emitirá um novo token de atualização para cada solicitação de token. <a>Saiba mais</a>',
  ...
  app_domain_description_1:
    'Sinta-se à vontade para utilizar o seu domínio com {{domain}} alimentado pela MyEyesID, que é permanentemente válido.',
  ...
  third_party_settings_description:
    'Integre aplicações de terceiros com a MyEyesID como seu Provedor de Identidade (IdP) usando OIDC / OAuth 2.0, apresentando uma tela de consentimento para autorização do utilizador.',
  ...
  permissions: {
    ...
    permission_description_tips:
      'Quando o MyEyesID é usado como um Prestador de Identidades (IdP) para autenticação em aplicações de terceiros, e os utilizadores são solicitados para autorização, esta descrição aparece no ecrã de consentimento.',
  ...
  saml_idp_name_id_format: {
    ...
    persistent_description: 'Usar o ID de utilizador do MyEyesID como ID de Nome',
    transient: 'Transitório',
    transient_description: 'Usar um ID de utilizador único como ID de Nome',
    unspecified: 'Não especificado',
    unspecified_description: 'Usar o ID de utilizador do MyEyesID como ID de Nome',
  ...
  saml_app_attribute_mapping: {
    ...
    description:
      'Adicione mapeamentos de atributos para sincronizar o perfil do utilizador do MyEyesID para a sua aplicação.',
    col_myeyesid_claims: 'Valor do MyEyesID',
