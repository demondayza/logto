const enterprise_sso_details = {
  ...
  service_provider_property_description:
    'Kimlik sağlayıcınızda {{protocol}} kullanarak bir uygulama entegrasyonu kurun. MyEyesID tarafından sağlanan ayrıntıları girin.',
  attribute_mapping_title: 'Özellik eşlemeleri',
  attribute_mapping_description:
    'Kimlik sağlayıcıdan kullanıcı profillerini kullanıcı özelliği eşlemesini yapılandırarak MyEyesID tarafına senkronize edin.',
  saml_preview: {
  ...
  idp_initiated_auth_config: {
    ...
    redirect_uri_tooltip:
      "Başarılı oturum açmadan sonra kullanıcıları yönlendirmek için kullanılacak yönlendirme URI'si. MyEyesID, bu URI'yi yetkilendirme isteğinde OIDC yönlendirme URI'si olarak kullanacaktır. Daha iyi güvenlik için IdP başlatmalı SSO kimlik doğrulama akışı için özel bir URI kullanın.",
  ...
  trust_unverified_email_tip:
    "Entra ID (OIDC) bağlayıcısı `email_verified` bilgisini döndürmez, bu da Azure'dan alınan e-posta adreslerinin doğrulandığının garanti edilmediği anlamına gelir. Varsayılan olarak, MyEyesID doğrulanmamış e-posta adreslerini kullanıcı profiline senkronize etmeyecektir. Bu seçeneği yalnızca Entra ID dizininden gelen tüm e-posta adreslerine güveniyorsanız etkinleştirin.",
  trust_unverified_email_tip_oidc:
    "OIDC bağlayıcısı `email_verified` iddiasını döndürmeyebilir; bu, IdP'den gelen e-posta adreslerinin doğrulandığının garanti edilmediği anlamına gelir. Varsayılan olarak, MyEyesID doğrulanmamış e-posta adreslerini kullanıcı profiline senkronize etmez. Bu seçeneği yalnızca IdP'den gelen tüm e-posta adreslerine güveniyorsanız etkinleştirin.",
  offline_access: {
