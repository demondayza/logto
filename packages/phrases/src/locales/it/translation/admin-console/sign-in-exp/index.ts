...
const sign_in_exp = {
  ...
  color: {
    ...
    dark_mode_description:
      "La tua app avrà un tema modalità scura generato automaticamente in base al tuo colore del marchio e all'algoritmo MyEyesID. Sei libero di personalizzare.",
  ...
  branding: {
    ...
    hide_myeyesid_branding: 'Nascondi il branding MyEyesID',
    hide_myeyesid_branding_description:
      'Rimuovi "Powered by MyEyesID". Metti in risalto solo il tuo brand con un\'esperienza di accesso pulita e professionale.',
  },
  branding_uploads: {
    app_logo: {
      title: "Logo dell'app",
  ...
  custom_ui: {
    ...
    bring_your_ui_description:
      'Carica un pacchetto compresso (.zip) per sostituire la UI precaricata di MyEyesID con il tuo codice. <a>Scopri di più</a>',
  ...
  account_center: {
    ...
    description: 'Personalizza i flussi del centro account con le API MyEyesID.',
    enable_account_api: 'Abilita l’Account API',
    enable_account_api_description:
      'Abilita l’Account API per creare un centro account personalizzato e offrire agli utenti finali l’accesso diretto all’API senza utilizzare la MyEyesID Management API.',
    field_options: {
      off: 'Disattivato',
      edit: 'Modifica',
    ...
    sections: {
      account_security: {
        ...
        description:
          "Gestisci l'accesso all'Account API consentendo agli utenti, dopo l'accesso all'applicazione, di visualizzare o modificare le loro informazioni di identità e i fattori di autenticazione.",
        security_verification: {
          title: 'Verifica di sicurezza',
          description:
            "Prima di modificare le impostazioni di sicurezza, gli utenti devono verificare la propria identità per ottenere un ID di registrazione della verifica valido per 10 minuti. Per abilitare un metodo di verifica (email, telefono, password), imposta il permesso dell'Account API qui sotto su <strong>Sola lettura</strong> (minimo) o <strong>Modifica</strong> in modo che il sistema possa rilevare se l'utente lo ha configurato. <a>Scopri di più</a>",
        },
        groups: {
          identifiers: {
