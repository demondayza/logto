const domain = {
  ...
  update_endpoint_notice:
    'N’oubliez pas de mettre à jour le domaine pour l’URI de rappel du connecteur social et le point de terminaison MyEyesID dans votre application si vous souhaitez utiliser un domaine personnalisé pour les fonctionnalités.',
  error_hint:
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        'Si vous avez configuré ce domaine personnalisé dans votre fournisseur de connecteur social ou le point de terminaison de l’application avant, vous devrez d’abord modifier l’URI vers le domaine MyEyesID par défaut "<span>{{domain}}</span>". Ceci est nécessaire pour que le bouton de connexion sociale fonctionne correctement.',
  ...
  default: {
    ...
    default_domain_description:
      'MyEyesID offre un domaine par défaut préconfiguré, prêt à être utilisé sans aucune configuration supplémentaire. Ce domaine par défaut sert de solution de secours même si vous avez activé un domaine personnalisé.',
    default_domain_field: 'Domaine par défaut de MyEyesID',
  },
