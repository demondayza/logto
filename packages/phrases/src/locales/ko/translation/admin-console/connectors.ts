const connectors = {
  ...
  placeholder_description:
    'MyEyesID는 널리 사용되는 다양한 소셜 로그인 커넥터를 제공하고 있으며, 표준 프로토콜을 사용하여 자신만의 커넥터를 만들 수도 있어요.',
  ...
  guide: {
    ...
    target_tooltip:
      'MyEyesID의 소셜 연동에서의 "공급자"는 소셜 정보의 원천을 뜻해요. MyEyesID의 디자인은 충돌을 피하기 위해서 같은 "공급자"를 허용하지 않아요. 연동을 추가한 후에는 값을 변경할 수 없으므로 주의해주세요. <a>자세히 알아보기</a>',
    target_conflict:
    ...
    callback_uri_custom_domain_description:
      'MyEyesID에서 여러 <a>사용자 지정 도메인</a>을 사용하는 경우 각 도메인에서 소셜 로그인이 동작하도록 모든 해당 콜백 URI를 IdP에 반드시 추가하세요.\n\n기본 MyEyesID 도메인 (*.myeyesid.app)은 항상 유효하므로 해당 도메인에서의 로그인을 지원하려는 경우에만 포함하세요.',
