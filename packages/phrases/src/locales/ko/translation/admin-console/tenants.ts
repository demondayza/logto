const tenants = {
  ...
  settings: {
    ...
    tenant_instance_description:
      '테넌트가 호스팅될 위치를 선택하세요. 공용 공유 인프라를 위해 MyEyesID Cloud 를 선택하거나, 전용 리소스를 위한 개별 인스턴스를 선택하세요.',
    ...
    enterprise_sso_description:
      '유료 플랜에서 사용 가능합니다. 엔터프라이즈 SSO를 활성화하여 모든 구성원이 조직의 ID 공급자를 사용하여 MyEyesID Cloud 콘솔에 로그인할 수 있도록 하려면 문의해 주세요.',
  },
  full_env_tag: {
    development: '개발',
    production: '프로드',
  ...
  create_modal: {
    ...
    tenant_id_tip:
      '테넌트 ID를 사용자 정의합니다. 비워두면 MyEyesID가 기본 ID를 생성합니다. 테넌트 ID는 생성 후 변경할 수 없습니다.',
  },
  ...
  tenant_landing_page: {
    ...
    description:
      'MyEyesID 를 사용하여 프로젝트를 구성하려면 새 테넌트를 만드세요. 로그아웃하거나 계정을 삭제하려면 오른쪽 상단 모서리에있는 아바타 버튼을 클릭하세요.',
