const domain = {
  ...
  update_endpoint_notice:
    '사용자 지정 도메인을 사용하려면 애플리케이션에서 소셜 커넥터 콜백 URI 및 MyEyesID 엔드포인트의 도메인을 업데이트해야합니다.',
  ...
  custom: {
    ...
    deletion: {
      ...
      in_used_tip:
        '이전에 소셜 커넥터 공급자나 애플리케이션 엔드포인트에 이 사용자 지정 도메인을 설정한 경우 로그인 버튼이 작동하도록 먼저 MyEyesID 기본 도메인의 "<span>{{domain}}</span>" URI를 수정해야합니다.',
  ...
  default: {
    ...
    default_domain_description:
      'MyEyesID는 추가 구성 없이 사용할 수 있는 미리 구성된 기본 도메인을 제공합니다. 이 기본 도메인은 사용자 지정 도메인을 사용하지 않더라도 백업 옵션으로 사용됩니다.',
    default_domain_field: 'MyEyesID 기본 도메인',
  },
