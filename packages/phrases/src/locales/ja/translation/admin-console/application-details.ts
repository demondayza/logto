const application_details = {
  ...
  settings_description:
    '「アプリケーション」とは、ユーザー情報にアクセスしたり利用者に代わって操作したりできる登録済みのソフトウェアやサービスを指します。アプリケーションは、MyEyesID に誰が何を要求しているのかを認識させ、サインインと権限の処理を担います。認証に必要な項目を入力してください。',
  integration: '統合',
  integration_description:
    "Deploy with MyEyesID secure workers, powered by Cloudflare's edge network for top-tier performance and 0ms cold starts worldwide.",
  ...
  always_issue_refresh_token_label:
    'この設定を有効にすると、MyEyesID は、認証要求に「prompt = consent」が提示されたかどうかにかかわらず、常に Refresh Token を発行することができます。ただし、OpenID Connect と互換性がないため、必要でない限りこのプラクティスは推奨されず、問題が発生する可能性があります。',
  ...
  rotate_refresh_token_label:
    '有効にすると、MyEyesID は、元の TTL の 70％ が経過したときまたは特定の条件が満たされた場合、トークン要求で新しい Refresh Token を発行します。<a>詳細を見る</a>',
  rotate_refresh_token_label_for_public_clients:
    '有効にすると、MyEyesID は各トークンリクエストに対して新しいリフレッシュトークンを発行します。<a>詳細を見る</a>',
  ...
  app_domain_description_1:
    'MyEyesID によって提供される {{domain}} を使用して、ドメインを自由に利用できます。これは永久に有効です。',
  app_domain_description_2:
    'MyEyesID によって提供される <domain>{{domain}}</domain> を自由に利用できます。これは永久に有効です。',
  ...
  third_party_settings_description:
    'OIDC / OAuth 2.0 を使用して MyEyesID をアイデンティティプロバイダ（IdP）として、サードパーティアプリケーションと統合し、ユーザー承認のための同意画面を提供します。',
  ...
  permissions: {
    ...
    permission_description_tips:
      'MyEyesID がサードパーティアプリケーションの認証プロバイダ（IdP）として使用され、ユーザーに承認を要求される場合、この説明が同意画面に表示されます。',
    user_title: 'ユーザー',
    user_description:
  ...
  saml_idp_name_id_format: {
    ...
    persistent: '永続的',
    persistent_description: 'MyEyesID ユーザー ID を Name ID として使用',
    transient: '一時的',
    transient_description: '一回限りのユーザー ID を Name ID として使用',
    unspecified: '未指定',
    unspecified_description: 'MyEyesID ユーザー ID を Name ID として使用',
