const enterprise_sso_details = {
  ...
  service_provider_property_description:
    'アイデンティティプロバイダーを使用して {{protocol}} でアプリケーション統合を設定します。MyEyesIDが提供する詳細情報を入力してください。',
  attribute_mapping_title: '属性マッピング',
  attribute_mapping_description:
    'アイデンティティプロバイダーまたはMyEyesID側でユーザー属性マッピングを設定して、ユーザープロファイルを同期します。',
  saml_preview: {
  ...
  trust_unverified_email_tip:
    'Entra ID (OIDC) コネクタは `email_verified` クレームを返さないため、Azure からのメールアドレスは確認済みであることは保証されません。デフォルトでは、MyEyesID は未検証のメールアドレスをユーザープロファイルに同期しません。Entra ID ディレクトリからのすべてのメールアドレスを信頼する場合にのみ、このオプションを有効にしてください。',
  trust_unverified_email_tip_oidc:
    'OIDC コネクタは `email_verified` クレームを返さない場合があり、IdP からのメールアドレスが検証済みである保証はありません。既定では、MyEyesID は未検証のメールアドレスをユーザープロファイルに同期しません。IdP のすべてのメールアドレスを信頼できる場合にのみ有効化してください。',
  offline_access: {
    label: 'アクセス トークンを更新',
    description:
