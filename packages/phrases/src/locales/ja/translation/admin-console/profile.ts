const profile = {
  ...
  delete_account: {
    ...
    p: {
      ...
      remove_all_data:
        'アカウントを削除すると、MyEyesID Cloud に関するすべてのデータが永久に削除されます。続行する前に、重要なデータをバックアップしてください。',
      confirm_information:
    ...
    try_again_later:
      '後でもう一度試してください。問題が解決しない場合は、リクエスト ID を持って MyEyesID チームに連絡してください。',
  ...
  mfa: {
    title: '二要素認証',
    section_description:
      '認証アプリ、パスキー、またはバックアップコードを使用して、アカウントにセキュリティを追加します。',
    two_step_verification: '2段階認証',
    not_configured: '未設定',
    configured: '設定済み',
    passkeys_count_one: '{{count}} 個のパスキー',
    passkeys_count_other: '{{count}} 個のパスキー',
    backup_codes_count_one: '{{count}} 個のバックアップコードが利用可能',
    backup_codes_count_other: '{{count}} 個のバックアップコードが利用可能',
    add: '追加',
    view: '表示',
    remove: '削除',
  },
