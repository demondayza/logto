import { css, html, type TemplateResult } from 'lit';
import { customElement } from 'lit/decorators.js';
import { when } from 'lit/directives/when.js';

import passwordIcon from '../icons/password.svg';

import { MyEyesIDProfileItemElement } from './MyEyesIDProfileItemElement.js';

const tagName = 'myeyesid-user-password';

@customElement(tagName)
export class MyEyesIDUserPassword extends MyEyesIDProfileItemElement {
  static tagName = tagName;

  static styles = css`
    .status {
      display: flex;
      align-items: center;
      gap: var(--myeyesid-spacing-sm);
    }

    .status-dot {
      width: 8px;
      height: 8px;
      border-radius: 50%;
      background-color: var(--myeyesid-color-container-on-success);
    }
  `;

  protected isAccessible(): boolean {
    // The password is always accessible
    return true;
  }

  protected getItemLabelInfo() {
    return {
      icon: passwordIcon,
      label: 'Password',
    };
  }

  protected renderContent(): TemplateResult {
    const { hasPassword } = this.accountContext?.userProfile ?? {};
    return when(
      hasPassword,
      () =>
        html`<div slot="content">
          <div class="status">
            <span class="status-dot"></span>
            Configured
          </div>
        </div>`
    );
  }
}

declare global {
  // eslint-disable-next-line @typescript-eslint/consistent-type-definitions
  interface HTMLElementTagNameMap {
    [tagName]: MyEyesIDUserPassword;
  }
}
