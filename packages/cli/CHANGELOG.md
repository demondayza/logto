...
- cb261024b: fix cli add offical connectors command missing connectors bug
  ...
  Fix the bug when running the cli commend `myeyesid connectors add --official`, only 8 connectors are fetched from npm registry.
...
- 0183d0c33: split translate command from @myeyesid/cli and create a standalone package

  The "translate" command has greatly increased the size of the "@myeyesid/cli" package, as it involves TypeScript code manipulation and has to use "typescrpt" as a "dependency".
  In fact, only a small number of developers who want to contribute MyEyesID will use this command, so we believe it's best to separate the less frequently used "translate" command from the "cli" package to keep it small and simple.

  Please also be noted that this change is actually a breaking change for those who use the "translate" command. However, the CLI has to be bundle-released with the "MyEyesID" open-source distribution, and we feel it is still too early to make a major version bump for MyEyesID. Therefore, the "minor" bump is used this time.
...
- 2d0502a42: add new cli command to setup proxy for developing and debugging custom ui locally
  ...
  This command will establish a proxy tunnel between the following 3 entities together: your MyEyesID cloud auth services, your application, and your custom sign-in UI.

  Assuming you have a custom sign-in page running on `http://localhost:4000`.
  Then you can execute the command this way:

  ```bash
  npm cli proxy --endpoint https://<tenant-id>.myeyesid.app --port 9000 --experience-uri http://localhost:4000
  ```
  ...
  ```bash
  npm cli proxy --endpoint https://<tenant-id>.myeyesid.app --port 9000 --experience-path /path/to/your/custom/ui
  ```
  ...
  Finally, run your application and set its MyEyesID endpoint to the proxy address `http://localhost:9000/` instead.
...
- fde330a8b: add `translate sync-keys` command
  ...
  This command is helpful for syncing keys from one language to another. Run `myeyesid translate sync-keys --help` for details.
...
- 3c84d81ff: add `myeyesid connector link` command to link local connectors without downloading from the remote
- 736d6d212: `myeyesid translate sync` to create missing files or translate untranslated phrases across all existing languages
...
- f41fd3f05: Add `myeyesid database alteration rollback` command for running `down()` alteration scripts
...
- 3ff2e90cd: **CLI**
  ...
  **Rotate your private or secret key**
  ...
  Run `myeyesid db config rotate help` for detailed usage.
  ...
  **Trim the private or secret key you don't need**
  ...
  If you want to remove multiple keys at once, just append a number to the command. E.g. `myeyesid db config trim oidc.cookieKeys 3`.

  Run `myeyesid db config trim help` for detailed usage.
...
- f41fd3f0: Add `myeyesid database alteration rollback` command for running `down()` alteration scripts
...
**Note:** Version bump only for package @myeyesid/cli

## [1.0.0-beta.11](https://github.com/myeyesid-io/myeyesid/compare/v1.0.0-beta.10...v1.0.0-beta.11) (2022-10-19)

### Features

- `npm create` compatibility ([a5cd73d](https://github.com/myeyesid-io/myeyesid/commit/a5cd73d961766c7c72180795051feabe9793fc7d))
...
- **cli:** `db seed` command ([5c7000d](https://github.com/myeyesid-io/myeyesid/commit/5c7000ddc30e316bd17f34d71d51c17016efec76))
- **cli:** add `download-url` option for install ([5dda0a6](https://github.com/myeyesid-io/myeyesid/commit/5dda0a6dd0e04468c078e3581e68a614ce23404c))
- **cli:** add connector command ([4ccbe4a](https://github.com/myeyesid-io/myeyesid/commit/4ccbe4ac6566aff0db1cd98a74441640677f6060))
- **cli:** command `init/i/install` ([f05691b](https://github.com/myeyesid-io/myeyesid/commit/f05691b4319279a49bf0bc87ba656b7990d52e53))
- **cli:** database config command ([0eb306a](https://github.com/myeyesid-io/myeyesid/commit/0eb306a61cf88b8be3be86852cb66b1d99ad713f))
- **cli:** get/set db config key ([0eff1e3](https://github.com/myeyesid-io/myeyesid/commit/0eff1e3591129802f3e9b3286652ef6fc8619cf5))
- **cli:** list connectors ([dcb9142](https://github.com/myeyesid-io/myeyesid/commit/dcb91428e6ef1021e383270e66d3e67bfc83e593))
- **cli:** remove connectors ([7d257c4](https://github.com/myeyesid-io/myeyesid/commit/7d257c45bfa37298c287b3ac867acd0606c4f028))

### Bug Fixes

- add publish config for public packages ([#2192](https://github.com/myeyesid-io/myeyesid/issues/2192)) ([38f664c](https://github.com/myeyesid-io/myeyesid/commit/38f664c27c4927970f40336b04154a5803cb5dc0))
- alteration script in dev ([9ebb3dd](https://github.com/myeyesid-io/myeyesid/commit/9ebb3ddfd963f6459ea332dbe1384058f77b453b))
- **cli:** `chooseAlterationsByVersion` should contain the last `next` version alteration script ([#2175](https://github.com/myeyesid-io/myeyesid/issues/2175)) ([fd50304](https://github.com/myeyesid-io/myeyesid/commit/fd50304f5ff5ffbc985695eaa73c1bc56b1ca061))
- **cli:** fix skip-when-exists option ([#2180](https://github.com/myeyesid-io/myeyesid/issues/2180)) ([4ce2073](https://github.com/myeyesid-io/myeyesid/commit/4ce207369228d404d919c491ba398acedcfd55fa))
