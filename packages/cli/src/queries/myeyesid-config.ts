import type { MyEyesIDConfig, MyEyesIDConfigKey, myeyesidConfigGuards } from '@myeyesid/schemas';
import { MyEyesIDConfigs } from '@myeyesid/schemas';
...
export const doesConfigsTableExist = async (pool: CommonQueryMethods) => {
  const { rows } = await pool.query<{ regclass: Nullable<string> }>(
    sql`select to_regclass(${MyEyesIDConfigs.table}) as regclass`
  );
...
export const updateValueByKey = async <T extends MyEyesIDConfigKey>(
  pool: CommonQueryMethods,
  tenantId: string,
  key: T,
  value: z.infer<(typeof myeyesidConfigGuards)[T]>
) =>
  pool.query(
    sql`
