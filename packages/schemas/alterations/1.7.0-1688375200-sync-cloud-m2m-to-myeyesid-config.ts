...
const cloudConnectionResourceIndicator = 'https://cloud.myeyesid.io/api';
...
const alteration: AlterationScript = {
  up: async (pool) => {
    ...
    const { rows: existingCloudConnections } = await pool.query<CloudConnectionConfig>(sql`
      select * from myeyesid_configs where key = ${cloudConnectionConfigKey}
    `);
    ...
    await pool.query(sql`
      insert into myeyesid_configs (tenant_id, key, value) values ${sql.join(
  ...
  down: async (pool) => {
    await pool.query(sql`
      delete from myeyesid_configs where key = ${cloudConnectionConfigKey}
    `);
  },
};
