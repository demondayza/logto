import { type IdTokenConfig } from '@myeyesid/schemas';
...
describe('id token config', () => {
  it('should get id token config successfully', async () => {
    const config = await getIdTokenConfig();
    expect(config).toMatchObject(defaultIdTokenConfig);
  });

  it('should update id token config successfully', async () => {
