...
describe('myeyesid config', () => {
  ...
  it('should get OIDC keys successfully', async () => {
    const privateKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.PrivateKeys);
    const cookieKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.CookieKeys);
  ...
  it('should not be able to delete the only private key', async () => {
    const privateKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.PrivateKeys);
    expect(privateKeys).toHaveLength(1);
    await expectRejects(deleteOidcKey(MyEyesIDOidcConfigKeyType.PrivateKeys, privateKeys[0]!.id), {
    ...
    const cookieKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.CookieKeys);
    expect(cookieKeys).toHaveLength(1);
    await expectRejects(deleteOidcKey(MyEyesIDOidcConfigKeyType.CookieKeys, cookieKeys[0]!.id), {
      code: 'oidc.key_required',
      status: 422,
    });
  });
  ...
  it('should rotate OIDC keys successfully', async () => {
    const existingPrivateKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.PrivateKeys);
    const newPrivateKeys = await rotateOidcKeys(
      MyEyesIDOidcConfigKeyType.PrivateKeys,
    ...
    const existingCookieKeys = await getOidcKeys(MyEyesIDOidcConfigKeyType.CookieKeys);
    const newCookieKeys = await rotateOidcKeys(MyEyesIDOidcConfigKeyType.CookieKeys);
  ...
  it('should only keep 2 recent OIDC keys', async () => {
    const privateKeys = await rotateOidcKeys(MyEyesIDOidcConfigKeyType.PrivateKeys); // Defaults to 'EC' algorithm
