import {
  ...
  type MyEyesIDOidcConfigKeyType,
...
export const getOidcKeys = async (keyType: MyEyesIDOidcConfigKeyType) =>
  authedAdminApi.get(`configs/oidc/${keyType}`).json<OidcConfigKeysResponse[]>();

export const deleteOidcKey = async (keyType: MyEyesIDOidcConfigKeyType, id: string) =>
  authedAdminApi.delete(`configs/oidc/${keyType}/${id}`);

export const rotateOidcKeys = async (
  keyType: MyEyesIDOidcConfigKeyType,
