...
import { ConnectorError, ConnectorErrorCodes, type SocialUserInfo } from '@myeyesid/connector-kit';
  ...
  it('should get a valid uri with custom scope', async () => {
    ...
    expect(authorizationUri).toEqual(
      `${authorizationEndpoint}?${new URLSearchParams({
        response_type: 'code',
        client_id: '<client-id>',
        scope: 'custom_scope', // We add openid to the scopes if not present always
        redirect_uri: 'http://localhost:3000/callback',
        state: 'some_state',
      }).toString()}`
    );
