...
import type { AmazonConfig } from './types.js';
import {
  amazonConfigGuard,
...
export const getAccessToken = async (config: AmazonConfig, code: string, redirectUri: string) => {
...
const getUserInfo =
  ...
  async (data, getSession) => {
    ...
    try {
      ...
      const { user_id, name, email } = userInfoResult.data;
...
const createAmazonConnector: CreateConnector<SocialConnector> = async ({ getConfig }) => {
  return {
    metadata: defaultMetadata,
    type: ConnectorType.Social,
    configGuard: amazonConfigGuard,
    getAuthorizationUri: getAuthorizationUri(getConfig),
    getUserInfo: getUserInfo(getConfig),
  };
};

export default createAmazonConnector;
