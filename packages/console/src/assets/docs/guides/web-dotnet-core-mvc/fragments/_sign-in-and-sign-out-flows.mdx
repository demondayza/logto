...
1. **CallbackPath**: The URI that MyEyesID will redirect the user back to after the user has signed in (the "redirect URI" in MyEyesID)
2. **RedirectUri**: The URI that will be redirected to after necessary actions have been taken in the MyEyesID authentication middleware.
...
graph LR
  ...
  A(Sign-in path) -->|Redirect to| B(MyEyesID)
...
| -------------------------------- | --------------------- |
| MyEyesID redirect URI               | CallbackPath          |
| MyEyesID post sign-out redirect URI | SignedOutCallbackPath |
