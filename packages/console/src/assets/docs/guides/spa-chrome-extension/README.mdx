...
<NpmLikeInstallation packageName="@myeyesid/chrome-extension" />
...
sequenceDiagram
  ...
  participant C as MyEyesID sign-in experience

  A->>B: Invokes sign-in
  B->>C: Redirects to MyEyesID
  C->>C: User signs in
...
MyEyesID SDK requires the following permissions in the `manifest.json`:

```json title="manifest.json"
{
  "permissions": ["identity", "storage"],
  "host_permissions": ["https://*.myeyesid.app/*"]
}
```
...
- `host_permissions`: Required for the MyEyesID SDK to communicate with the MyEyesID APIs.

<InlineNotification>
If you are using a custom domain on MyEyesID Cloud, you need to update the `host_permissions` to match your domain.
</InlineNotification>

### Set up a background script (service worker)

In your Chrome extension's background script, initialize the MyEyesID SDK:

```js title="service-worker.js"
import MyEyesIDClient from '@myeyesid/chrome-extension';

export const myeyesidClient = new MyEyesIDClient({
  endpoint: '<your-myeyesid-endpoint>'
  appId: '<your-myeyesid-app-id>',
});
```

Replace `<your-myeyesid-endpoint>` and `<your-myeyesid-app-id>` with the actual values. You can find these values in the application page you just created in the MyEyesID Console.
...
chrome.runtime.onMessage.addListener((message, sender, sendResponse) => {
  ...
  if (message.action === 'signIn') {
    ...
    myeyesidClient.signIn(redirectUri).finally(sendResponse);
    return true;
  }

  if (message.action === 'signOut') {
    const redirectUri = chrome.identity.getRedirectURL();
    myeyesidClient.signOut(redirectUri).finally(sendResponse);
    return true;
  }
...
<Step title="Update MyEyesID application settings">

As we mentioned in the previous step, we need to update the MyEyesID application settings to allow the redirect URIs we just created (`https://<extension-id>.chromiumapp.org/callback`):
...
Finally, the CORS allowed origins should include the extension's origin (`chrome-extension://<extension-id>`). The SDK in Chrome extension will use this origin to communicate with the MyEyesID APIs.
...
Since Chrome provide unified storage APIs, rather than the sign-in and sign-out flow, all other MyEyesID SDK methods can be used in the popup page directly.

In your `popup.js`, you can reuse the `MyEyesIDClient` instance created in the background script, or create a new one with the same configuration:
...
// Or reuse the myeyesidClient instance created in the background script
import { myeyesidClient } from './service-worker.js';
```
...
const loadAuthenticationState = async () => {
  ...
  if (isAuthenticated) {
    const user = await myeyesidClient.getIdTokenClaims(); // { sub: '...', email: '...', ... }
...
- **Module resolution**: MyEyesID Chrome extension SDK is an ESM-only module.

See our [sample project](https://github.com/myeyesid-io/js/tree/HEAD/packages/chrome-extension-sample) for a complete example with TypeScript, Rollup, and other configurations.
