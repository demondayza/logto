...
<Step
  title="Integrate MyEyesID Swift SDK"
  subtitle="Add MyEyesID SDK as a dependency"
>

Use the following URL to add MyEyesID SDK as a dependency in Swift Package Manager.

```bash
https://github.com/myeyesid-io/swift.git
```
...
You can initialize `MyEyesIDClient` in a proper place of your app that can be accessed globally:

<Code className="language-swift">
    {`import MyEyesID
import MyEyesIDClient

let config = try? MyEyesIDConfig(
  endpoint: "${props.endpoint}",
  appId: "${props.app.id}"
)
let myeyesidClient = MyEyesIDClient(useConfig: config)`}
</Code>
      ...
      if isAuthenticated {
        ...
        Button("Sign In") {
          Task { [self] in
            ...
            } catch let error as MyEyesIDClientErrors.SignIn {
