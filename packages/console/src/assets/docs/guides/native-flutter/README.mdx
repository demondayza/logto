...
In your application, you can use the `logtoClient.signIn` method to initiate the sign-in process. This method will redirect the user to the MyEyesID sign-in page and handle the authentication flow.
...
In MyEyesID SDK, you can use `logtoClient.isAuthenticated` to check the authentication status, if the user is signed in, the value will be `true`, otherwise, the value will be `false`.
