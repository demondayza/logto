...
You can install the `myeyesid_dart_sdk package` directly using the pub package manager.
Run the following command under your project root:

```sh
flutter pub get myeyesid_dart_sdk
```
...
```sh
git clone https://github.com/myeyesid-io/dart
```
...
| MyEyesID SDK version | Dart SDK version  | Dart 3.0 compatible |
| ----------------- | ----------------- | ------------------- |
...
Behind the scenes, this SDK uses [flutter_web_auth_2](https://pub.dev/packages/flutter_web_auth_2) to authenticate users with MyEyesID. This package provides a simple way to authenticate users with MyEyesID using the system webview or browser.
...
- Android: Register the callback url on Android
  ...
  In order to capture the callback url from MyEyesID's sign-in web page, you will need to register your sign-in redirectUri to your `AndroidManifest.xml` file.
...
Import the `myeyesid_dart_sdk` package and initialize the `MyEyesIDClient` instance at the root state of your application.

<Code className="language-dart" title="lib/main.dart">
{`import 'package:myeyesid_dart_sdk/myeyesid_dart_sdk.dart';
...
class MyApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return const MaterialApp(
      title: 'Flutter Demo',
      home: MyHomePage(title: 'MyEyesID Demo Home Page'),
    );
  }
}
...
class _MyHomePageState extends State<MyHomePage> {
  late MyEyesIDClient myeyesidClient;
  ...
  // highlight-start
  // MyEyesIDConfig
  final myeyesidConfig = const MyEyesIDConfig(
    endpoint: "<your-myeyesid-endpoint>",
    appId: "<your-app-id>"
  );

  void _init() {
    myeyesidClient = MyEyesIDClient(
      config: myeyesidConfig,
...
- For Android, in order to capture the callback url from MyEyesID's sign-in web page, you will need to register your sign-in redirectUri to the `AndroidManifest.xml`.
    ...
    <activity android:name="com.linusu.flutter_web_auth.CallbackActivity" android:exported="true">
      <intent-filter android:label="flutter_web_auth">
        ...
        <category android:name="android.intent.category.DEFAULT"/>
        <category android:name="android.intent.category.BROWSABLE"/>
        <data android:scheme="io.myeyesid"/>
      </intent-filter>
    </activity>
    ```
...
Just like signing in, users should be redirected to MyEyesID for signing out of the shared session. Once finished, it would be great to redirect the user back to your website. For example, add  {`${props.defaultUri ?? defaultRedirectUri}`} as the post sign-out redirect URI below.
...
In your application, you can use the `myeyesidClient.signIn` method to initiate the sign-in process. This method will redirect the user to the MyEyesID sign-in page and handle the authentication flow.

<Code className="language-dart" title="lib/main.dart">
{`class _MyHomePageState extends State<MyHomePage> {
  // ...
  final redirectUri = 'io.myeyesid://callback';
  ...
  Widget build(BuildContext context) {
    ...
    Widget signInButton = TextButton(
      onPressed: () async {
        // highlight-start
        await myeyesidClient.signIn(redirectUri);
        render();
        // highlight-end
      },
...
{`class _MyHomePageState extends State<MyHomePage> {
  ...
  final postSignOutRedirectUri = 'io.myeyesid://home';
  ...
  Widget build(BuildContext context) {
    ...
    Widget signOutButton = TextButton(
      onPressed: () async {
        // highlight-start
        await myeyesidClient.signOut(postSignOutRedirectUri);
        render();
        // highlight-end
      },
...
In MyEyesID SDK, you can use `myeyesidClient.isAuthenticated` to check the authentication status, if the user is signed in, the value will be `true`, otherwise, the value will be `false`.
...
class _MyHomePageState extends State<MyHomePage>{
  ...
  void render() async {
    if (await myeyesidClient.isAuthenticated()) {
      setState(() {
