...
2. Search for "MyEyesID", or enter `https://wordpress.org/plugins/myeyesid/` in the search box.
3. Click **Install Now**.
4. Click **Activate**.
...
1. Download the MyEyesID WordPress plugin from one of the following links:
   - [Latest release](https://github.com/myeyesid-io/wordpress/releases): Download the file which name in the format of `myeyesid-plugin-<version>.zip`.
   - [WordPress plugin directory](https://wordpress.org/plugins/myeyesid/): Download the file by clicking the **Download** button.
...
Now you should be able to see the MyEyesID menu in your WordPress admin panel sidebar. Click **MyEyesID** > **Settings** to configure the plugin.

The minimum configuration to get started for the plugin is:

- MyEyesID endpoint: <code>{props.endpoint}</code>
...
The redirect URI is the URL to which MyEyesID will redirect users after they have authenticated; and the post sign-out redirect URI is the URL to which MyEyesID will redirect users after they have logged out.
...
sequenceDiagram
    ...
    participant MyEyesID

    User->>WordPress: Visit WordPress login page
    WordPress->>MyEyesID: Redirect to MyEyesID for authentication
    MyEyesID->>User: Prompt for authentication
    User->>MyEyesID: Authenticate
    MyEyesID->>WordPress: Redirect back to WordPress with authentication data
...
sequenceDiagram
    ...
    WordPress->>MyEyesID: Redirect to MyEyesID for sign-out
    MyEyesID->>MyEyesID: Sign-out successful
    MyEyesID->>WordPress: Redirect back to WordPress
...
In our case, we need to configure both redirect URIs in your MyEyesID Console.

To find the redirect URI, go to the **MyEyesID** > **Settings** page in your WordPress admin panel. You'll see the **Redirect URI** and **Post sign-out redirect URI** fields.
...
Now you can test your MyEyesID integration in your WordPress website:
...
3. The page should redirect you to the MyEyesID sign-in page.
...
7. You should be redirected to the MyEyesID sign-out page, then back to your WordPress website.
