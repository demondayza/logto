...
To proceed, you'll need to integrate the MyEyesID SDK package (for example, `@logto/react`) into your client application. This application might differ from your Express.js backend; for example, you might have a React app using Express.js as the backend API server.
...
To address this, we can employ role-based access control (RBAC). In MyEyesID, you can define roles and assign permissions to them. Consult [this tutorial](https://docs.logto.io/docs/recipes/rbac/) to learn how to define roles and permissions in MyEyesID.
