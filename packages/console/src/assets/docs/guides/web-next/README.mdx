...
1. `/api/logto/sign-in`: Sign in with MyEyesID.
2. `/api/logto/sign-in-callback`: Handle sign-in callback (redirect URI).
3. `/api/logto/sign-out`: Sign out with MyEyesID.
4. `/api/logto/user`: Check if user is authenticated with MyEyesID, if yes, return user info.
