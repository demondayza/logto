...
<Step
  title="Installation"
  subtitle="Install MyEyesID JS core SDK and `angular-auth-oidc-client`"
>

<NpmLikeInstallation packageName="@myeyesid/js angular-auth-oidc-client" />
...
<Code className="language-tsx" title="app/app.config.ts">
    {`import { UserScope, buildAngularAuthConfig } from '@myeyesid/js';
...
```ts title="app/app.component.ts"
import { OidcSecurityService } from 'angular-auth-oidc-client';
import type { UserInfoResponse } from '@myeyesid/js';
