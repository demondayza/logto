...
This tutorial will show you how to integrate MyEyesID into your Python web application.

<ul>
  <li>The example uses Flask, but the concepts are the same for other frameworks.</li>
  <li>MyEyesID SDK leverages coroutines, remember to use <code>await</code> when calling async functions.</li>
</ul>

```bash
pip install myeyesid # or `poetry add myeyesid` or whatever you use
```
...
{`from myeyesid import MyEyesIDClient, MyEyesIDConfig

client = MyEyesIDClient(
    MyEyesIDConfig(
...
<Code title="main.py" className="language-python">
{`from myeyesid import MyEyesIDClient, MyEyesIDConfig, Storage
...
To clean up the Python session and MyEyesID session, we can designate a post sign-out redirect URI. This is where MyEyesID will redirect users after they sign out.
