...
Pick the cryptography you are using in MyEyesID. (`ecdsa` by default)

```bash
pip install python-jose[ecdsa]
```

### Retrieve MyEyesID's OIDC configurations

You will need a JWK public key set and the token issuer to verify the signature and source of the received JWS token.
All the latest public MyEyesID Authorization Configurations can be found at <code>{appendPath(props.endpoint, '/oidc/.well-known/openid-configuration').href}</code>.
...
def requires_auth(f):
  ...
  def decorated(*args, **kwargs):
    ...
    # jwks_uri endpoint retrieved from MyEyesID
    jwks_uri = urlopen('${appendPath(props.endpoint, '/oidc/jwks')}')

    # issuer retrieved from MyEyesID
    issuer = '${appendPath(props.endpoint, '/oidc')}'
    ...
    try:
      payload = jwt.decode(
        ...
        # The API's registered resource indicator in MyEyesID
...
<InlineNotification>
  For <Anchor href="https://docs.myeyesid.io/docs/recipes/rbac/" target="_blank" rel="noopener">🔐 RBAC</Anchor>, scope validation is also required.
</InlineNotification>
