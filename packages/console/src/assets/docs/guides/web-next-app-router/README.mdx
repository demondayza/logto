...
<Code title="app/myeyesid.ts" className="language-ts">
    {`export const myeyesidConfig = {
...
import { handleSignIn } from '@myeyesid/next/server-actions';
import { redirect } from 'next/navigation';
import { NextRequest } from 'next/server';
import { myeyesidConfig } from '../myeyesid';

export async function GET(request: NextRequest) {
  const searchParams = request.nextUrl.searchParams;
  await handleSignIn(myeyesidConfig, searchParams);
...
Now let's add the sign-in and sign-out buttons in your hoem page. We need to call the server actions in SDK when needed. To help with this, use `getMyEyesIDContext` to fetch authentication status.

```tsx title="app/page.tsx"
import { getMyEyesIDContext, signIn, signOut } from '@myeyesid/next/server-actions';
import SignIn from './sign-in';
import SignOut from './sign-out';
import { myeyesidConfig } from './myeyesid';

const Home = () => {
  const { isAuthenticated, claims } = await getMyEyesIDContext(myeyesidConfig);
