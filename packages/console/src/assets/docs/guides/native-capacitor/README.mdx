...
import myeyesidSignInPage from './assets/myeyesid-sign-in-page.webp';
import myeyesidSignOutPage from './assets/myeyesid-sign-out-page.webp';
...
First, install MyEyesID Capacitor SDK and its peer dependencies:

<NpmLikeInstallation packageName="@myeyesid/capacitor @capacitor/browser @capacitor/app @capacitor/preferences" />

</Step>

<Step title="Init MyEyesID client">

Add the following code to your Capacitor project:

<Code className="language-ts">
    {`import MyEyesIDClient from '@myeyesid/capacitor';
...
<Code className="language-ts">
    {`const onClick = async () => {
  await myeyesidClient.signIn('${props.redirectUris[0] || defaultRedirectUri}');
  console.log(await myeyesidClient.isAuthenticated()); // true
  console.log(await myeyesidClient.getIdTokenClaims()); // { sub: '...', ... }
};`}
</Code>

Run the Capacitor app and click the sign-in button. A browser window will open, redirecting to the MyEyesID sign-in page.

<center>
  <img
    style={{ border: '1px solid #eaeaea', borderRadius: '16px' }}
    src={myeyesidSignInPage}
    alt="MyEyesID sign-in page"
    width="320"
  />
</center>

> If the user closes the browser window without completing the authentication flow, the Capacitor
> app will receive a `MyEyesIDClientError`.
...
const onClick = async () => {
  ...
  console.log(await myeyesidClient.isAuthenticated()); // false
};
```

The `signOut` method has an optional parameter for the post sign-out redirect URI. If it's not provided, the user will be redirected to the MyEyesID sign-out page:

<center>
  <img
    style={{ border: '1px solid #eaeaea', borderRadius: '16px' }}
    src={myeyesidSignOutPage}
    alt="MyEyesID sign-out page"
    width="320"
  />
</center>
...
<Code className="language-ts">
    {`const onClick = async () => {
  await myeyesidClient.signOut('${props.postLogoutRedirectUris[0] || defaultPostSignOutUri}');
};`}
</Code>
...
Then click the sign-out button, and the Capacitor app will be redirected to the MyEyesID sign-out page. It will automatically redirect back to the Capacitor app if the post sign-out redirect URI is configured.
