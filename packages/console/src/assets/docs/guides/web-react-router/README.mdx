...
<NpmLikeInstallation packageName="@myeyesid/react-router" />
...
import { makeMyEyesIDReactRouter } from '@myeyesid/react-router';

const sessionStorage = createCookieSessionStorage({
  cookie: {
    name: 'myeyesid-session',
    maxAge: 14 * 24 * 60 * 60,
    secrets: ['${generateStandardSecret()}'], // Auto-generated 32 digit secret
  },
});

export const myeyesid = makeMyEyesIDReactRouter(
  {
...
<RedirectUrisWeb defaultRedirectUri={`${defaultBaseUrl}api/myeyesid/callback`} />

</Step>

<Step title="Configure routes and mount authentication routes">

The SDK ships with a convenient function that mounts the authentication routes: sign-in, sign-in callback and the sign-out route. First, create a file `app/routes/api.myeyesid.$action.ts`:

```ts title="app/routes/api.myeyesid.$action.ts"
import { myeyesid } from '../services/auth.server';

export const loader = myeyesid.handleAuthRoutes({
  'sign-in': {
    path: '/api/myeyesid/sign-in',
    redirectBackTo: '/api/myeyesid/callback',
  },
  'sign-in-callback': {
    path: '/api/myeyesid/callback',
    redirectBackTo: '/',
  },
  'sign-out': {
    path: '/api/myeyesid/sign-out',
    redirectBackTo: '/',
  },
  'sign-up': {
    path: '/api/myeyesid/sign-up',
    redirectBackTo: '/api/myeyesid/callback',
  },
});
```
...
export default [
  ...
  route("api/myeyesid/:action", "routes/api.myeyesid.$action.ts"),
...
When mounting the routes as described above, you can navigate your browser to `/api/myeyesid/sign-in` and you should be redirected to your MyEyesID instance where you have to authenticate then.
...
We have prepared the authentication routes, now let's implement the sign-in and sign-out buttons in your home page. We need to redirect the user to the sign-in or sign-out route when needed. To help with this, use `loader` to fetch authentication status from MyEyesID client.

```tsx title="app/routes/_index.tsx"
import { type MyEyesIDContext } from '@myeyesid/react-router';
import { Link, type LoaderFunctionArgs } from 'react-router';

import { myeyesid } from '../services/auth.server';

type LoaderResponse = {
  readonly context: MyEyesIDContext;
};

export const loader = async ({ request }: LoaderFunctionArgs) => {
  const context = await myeyesid.getContext({ getAccessToken: false })(request);
    ...
    <div>
      {isAuthenticated ? (
        <div>
          ...
          <form action="/api/myeyesid/sign-out" method="get">
            <button type="submit">Sign Out</button>
          </form>
        </div>
      ) : (
        <form action="/api/myeyesid/sign-in" method="get">
