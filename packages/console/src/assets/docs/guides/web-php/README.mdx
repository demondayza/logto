...
This tutorial will show you how to integrate MyEyesID into your PHP web application.
...
{`use MyEyesID\\Sdk\\MyEyesIDClient;
use MyEyesID\\Sdk\\MyEyesIDConfig;

$client = new MyEyesIDClient(
  new MyEyesIDConfig(
...
By default, the SDK uses the built-in PHP session to store the MyEyesID data. If you want to use other storage, you can pass a custom storage object as the second parameter:
...
After the user signs in, MyEyesID will redirect the user to the callback URL you set in the MyEyesID Console. In this example, we use `/callback` as the callback URL:
...
Now, whenever your users visit `/sign-in`, it will start a new sign-in attempt and redirect the user to the MyEyesID sign-in page.
...
To clean up the PHP session and MyEyesID session, we can designate a post sign-out redirect URI. This is where MyEyesID will redirect users after they sign out.
...
The post sign-out redirect URI is optional, and if not provided, the user will be redirected to a MyEyesID default page after a successful sign-out (without redirecting back to your application).

</Step>

<Step title="Handle authentication status">

In MyEyesID SDK, we can use `$client->isAuthenticated()` to check the authentication status, if the user is signed in, the value will be true, otherwise, the value will be false.
...
Route::get('/userinfo', function () {
  ...
  return (
    ...
    . "<br>"
    // Fetch user info from MyEyesID userinfo endpoint
...
Our data models are based on [JsonModel](https://github.com/myeyesid-io/php/blob/master/docs/api/classes/MyEyesID/Sdk/Models/JsonModel.md), which is safe to accept undefined keys while encoding or decoding JSON.
...
To learn more about scopes and claims, see [Get user information](https://docs.myeyesid.io/quick-starts/php/#get-user-information).
