...
<Step
  title="Add MyEyesID SDK as a dependency"
...
<Step
  title="Initialize MyEyesID client"
  subtitle="1 step"
>
...
In the file where you want to initialize the MyEyesID client (e.g. a base controller or a middleware), add the following code:

<Code className="language-ruby">
    {`require "myeyesid/client"

@client = MyEyesIDClient.new(
  config: MyEyesIDClient::Config.new(
    endpoint: "${props.endpoint}",
    app_id: "${props.app.id}",
    app_secret: "${props.secrets[0]?.value ?? props.app.secret}"
  ),
  navigate: ->(uri) { a_redirect_method(uri) },
  storage: MyEyesIDClient::SessionStorage.new(the_session_object)
)
end`}
</Code>
...
class SampleController < ApplicationController
  before_action :initialize_myeyesid_client

  private

  def initialize_myeyesid_client
    @client = MyEyesIDClient.new(
      config: MyEyesIDClient::Config.new(
        endpoint: "${props.endpoint}",
        app_id: "${props.app.id}",
      ...
      # Allow the client to redirect to other hosts (i.e. your MyEyesID tenant)
      navigate: ->(uri) { redirect_to(uri, allow_other_host: true) },
      # Controller has access to the session object
      storage: MyEyesIDClient::SessionStorage.new(session)
    )
  end
end`}
</Code>
...
Please refer to the `#id_token_claims` method in the [gemdocs](https://gemdocs.org/gems/myeyesid/latest) for more information.
