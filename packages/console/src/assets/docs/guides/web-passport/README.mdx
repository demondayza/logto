...
<UriInputField name="redirectUris" defaultValue="http://localhost:3000/api/auth/callback/myeyesid" />
...
This redirects to MyEyesID's session end URL, then back to the homepage.
...
app.get('/', (request: Request, response) => {
  ...
  if (user) {
    response.end(
      `<h1>Hello MyEyesID</h1><p>Signed in as ${JSON.stringify(
        user
      )}, <a href="/sign-out">Sign Out</a></p>`
    );
  } else {
    response.end(`<h1>Hello MyEyesID</h1><p><a href="/sign-in">Sign In</a></p>`);
  }
});
```
...
<Step title="Checkpoint: Test MyEyesID and Passport integration">
