...
<Step
  title="Install MyEyesID SDK"
>

Use your favorite package manager to install the MyEyesID SDK:

<NpmLikeInstallation packageName="@myeyesid/sveltekit" />
...
In your `hooks.server.ts` file, add the following code to inject the MyEyesID hook into your server:

<Code className="language-tsx" title="src/hooks.server.ts">
    {`import { handleMyEyesID } from '@myeyesid/sveltekit';

export const handle = handleMyEyesID(
  {
...
export const handle = sequence(handleMyEyesID, handleOtherHook);
```

Now you can access the MyEyesID client in the `locals` object. For TypeScript, you can add the type to `app.d.ts`:

```ts title="src/app.d.ts"
import type { MyEyesIDClient, UserInfoResponse } from '@myeyesid/sveltekit';
...
export const actions: Actions = {
  signIn: async ({ locals }) => {
    await locals.myeyesidClient.signIn('${props.redirectUris[0] ?? 'http://localhost:3000/callback'}');
  },
  signOut: async ({ locals }) => {
    await locals.myeyesidClient.signOut('${
