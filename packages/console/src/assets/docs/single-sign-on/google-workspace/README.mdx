...
3. Set the `Scopes` for your application. In order to retrieve the user's identity information and email address properly from the IdP, MyEyesID SSO connector need to grant the following scopes from the IdP:
...
2. Fill in the `Name` of your client application, `MyEyesID SSO Connector` for example. This will help you to identify the credential in the future.

3. Fill in the `Authorized redirect URIs` with the MyEyesID callback URI. This is the URI that the Google will redirect the user's browser after successful authentication. After a user successfully authenticates with the IdP, the IdP redirects the user's browser back to this designated URI along with an authorization code. MyEyesID will complete the authentication process based on authorization code received from this URI.

<OidcCallbackUri />

4. Fill in the `Authorized JavaScript origins` with the MyEyesID callback URI's origin. This ensures only your MyEyesID application can send requests to the Google OAuth server.

5. Click the `Create` button to create the OAuth credential.

</Step>

<Step index={3} title="Set up MyEyesID connector with the client credentials">
...
Copy the `client ID` and `client secret` and fill in the corresponding fields on the MyEyesID SSO connector `connection` tab.

Now you have successfully configured a Google Workspace SSO connector on MyEyesID.
...
**In MyEyesID Google Workspace connector:**
...
If you want to access [Google APIs](https://console.cloud.google.com/apis/library) and perform actions with user authorization, MyEyesID needs to get specific API scopes and store tokens.

1. Add the required scopes in your Google Cloud Console OAuth consent screen configuration and MyEyesID Google connector.
2. Enable **Store tokens for persistent API access** in MyEyesID Google connector. MyEyesID will securely [store Google access and refresh tokens in the Secret Vault](https://docs.myeyesid.io/secret-vault/federated-token-set/token-retrieval).
3. To ensure refresh tokens are returned, configure your MyEyesID Google connector to enable **Offline Access**.
