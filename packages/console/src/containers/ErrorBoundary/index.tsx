import { MyEyesIDClientError, MyEyesIDError, OidcError, MyEyesIDRequestError } from '@myeyesid/react';
 ...
 *
 * OIDC request error is globally converted to the MyEyesIDRequestError.
 * @see {@link @myeyesid/core/packages/core/src/middleware/koa-oidc-error-handler.ts}
 ...
 *
 * - Includes `useMyEyesID()` (depending on `<MyEyesIDProvider />`) to provide a "Sign in again" button for session expired errors.
...
class ErrorBoundary extends Component<Props, State> {
  ...
  render() {
    ...
    if (isInCallback()) {
      if (error instanceof MyEyesIDError && error.data instanceof OidcError) {
        return (
          <AppError
    ...
    if (error instanceof MyEyesIDError && error.code === 'crypto_subtle_unavailable') {
      return <AppError errorMessage={t('errors.insecure_contexts')} callStack={error.stack} />;
    }

    // Treat other MyEyesID errors and 401 responses as session expired
    if (
